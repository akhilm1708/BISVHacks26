...
export default function AudioUploader() {
  ...
  async function handleSubmit() {
    ...
    if (file.size > maxBytes) {
      setError(`File is too large. Please use a recording under ${MAX_FILE_SIZE_MB} MB.`)
      return
    }
    submitCountRef.current += 1
    const thisSubmit = submitCountRef.current
    setLoadingStep(1)
    setLoading(true)
    setError(null)
    setResult(null)
    ...
    try {
      ...
      if (thisSubmit !== submitCountRef.current) return
      setLoadingStep(2)
      ...
      if (thisSubmit === submitCountRef.current) setLoading(false)
    }
  }
